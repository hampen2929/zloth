...
class PRService:
    ...
    async def create(self, task_id: str, data: PRCreate) -> PR:
        ...
        if not run.working_branch:
            raise ValidationError(
                f"Run has no working branch: {data.selected_run_id}",
                details={"run_id": data.selected_run_id},
            )
        if not run.commit_sha:
            raise ValidationError(
                f"Run has no commits: {data.selected_run_id}",
                details={"run_id": data.selected_run_id},
            )
        ...
        pr_body = data.body or f"Generated by zloth\n\n{run.summary or ''}"
    ...
    async def sync_manual_pr(self, task_id: str, selected_run_id: str) -> PRSyncResult:
        ...
        if not run.working_branch:
            raise ValidationError(
                f"Run has no working branch: {selected_run_id}",
...
"""
    ...
    async def _execute_for_description(
        ...
        temp_file = Path(f"/tmp/zloth_pr_desc_{uuid.uuid4().hex}.md")

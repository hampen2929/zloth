-- zloth v0.1 PostgreSQL Schema
-- This schema is compatible with PostgreSQL 13+
...
CREATE TABLE IF NOT EXISTS runs (
    ...
    summary TEXT,
    patch TEXT,
    files_changed JSONB DEFAULT '[]'::jsonb,  -- JSON array of FileDiff
    logs JSONB DEFAULT '[]'::jsonb,           -- JSON array of log strings
    warnings JSONB DEFAULT '[]'::jsonb,       -- JSON array of warning strings
    error TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- Note: PostgreSQL doesn't support CHECK (id = 1) the same way,
-- so we use a UNIQUE constraint with a default value approach
...
CREATE TABLE IF NOT EXISTS user_preferences (
    ...
    default_branch_prefix TEXT,             -- Default branch prefix for work branches (e.g., "zloth")
...
CREATE TABLE IF NOT EXISTS backlog_items (
    ...
    target_files JSONB NOT NULL DEFAULT '[]'::jsonb,  -- JSON array
    implementation_hint TEXT,
    tags JSONB NOT NULL DEFAULT '[]'::jsonb,          -- JSON array
    subtasks JSONB NOT NULL DEFAULT '[]'::jsonb,      -- JSON array of SubTask
...
CREATE TABLE IF NOT EXISTS reviews (
    ...
    target_run_ids JSONB NOT NULL,  -- JSON array
...
CREATE TABLE IF NOT EXISTS ci_checks (
    ...
    workflow_run_id BIGINT,            -- PostgreSQL uses BIGINT for large integers
    sha TEXT,
    jobs JSONB,                        -- JSON: job_name -> result
    failed_jobs JSONB,                 -- JSON: list of CIJobResult
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    FOREIGN KEY (task_id) REFERENCES tasks(id),

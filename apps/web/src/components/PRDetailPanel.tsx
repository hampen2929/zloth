...
import type { PR, PRUpdateMode } from '@/types';
...
const UPDATE_OPTIONS: { id: PRUpdateMode; label: string; description: string }[] = [
...
export function PRDetailPanel({
  ...
  const handleUpdatePR = async (mode: PRUpdateMode) => {
    setIsDropdownOpen(false);
    setRegenerating(true);
    try {
      const updatedPR = await prsApi.regenerateDescription(taskId, pr.id, mode);
      setCurrentPR(updatedPR);
      const modeLabel = UPDATE_OPTIONS.find((opt) => opt.id === mode)?.label || mode;
      success(`PR ${modeLabel.toLowerCase()} updated successfully!`);
      onUpdate?.();
    } catch (err) {
            ...
            <div className="relative" ref={dropdownRef}>
              <div className="flex">
                ...
                >
                  ...
                  {regenerating ? 'Updating...' : 'Update PR'}
                </Button>
                <Button
                  variant="secondary"
                  size="sm"

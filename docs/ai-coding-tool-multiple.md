...
現在、zloth のトップページで New Task を作成する際、AI Coding Tool（Claude Code, Codex, Gemini CLI）は1つしか選択できない。本機能では、複数の AI Coding Tool を同時に選択し、並列実行できるようにする。
...
| `apps/api/src/zloth_api/domain/models.py` | Pydanticモデル |
| `apps/api/src/zloth_api/domain/enums.py` | Enum定義 |
| `apps/api/src/zloth_api/services/run_service.py` | ラン作成・実行サービス |
| `apps/api/src/zloth_api/routes/runs.py` | APIエンドポイント |
...
#### Python (apps/api/src/zloth_api/domain/models.py)

...
This document defines the design policy for placing git operations (worktree, add, commit, push, etc.) in tazuna.

## Current Design (as implemented)

```mermaid
graph TB
    subgraph tazuna["tazuna (Orchestrator)"]
        GS[GitService]
        EX[Executor]
        PR[PRService]
    end
...
A method where tazuna centrally manages git operations, allowing AI Agents to **only edit files**.
...
flowchart TD
    ...
    subgraph "tazuna Managed (Automatic)"
        B
        E
        G
        H
...
```python
# apps/api/src/tazuna_api/services/git_service.py
...
```python
# apps/api/src/tazuna_api/executors/base_executor.py

from abc import ABC, abstractmethod
from pathlib import Path
from collections.abc import Awaitable, Callable

from tazuna_api.domain.models import AgentConstraints
...
class PRService:
    ...
    async def create_from_run(
        ...
        pr_response = await self.github_service.create_pull_request(
            ...
            body=pr_data.body or f"Generated by tazuna\n\n{run.summary}",
        )
...
1. **Separation of Responsibilities**: AI Agent only edits files, tazuna centrally manages git operations
...
|-------|-----------|----------|
| Preparation | git worktree add | tazuna |
| Execution | File editing | AI Agent |
| Save | git add, commit, push | tazuna (automatic) |
| Create PR | GitHub API: Create PR | tazuna (on user request) |
| Update PR | Additional instruction → commit/push (auto-reflected) | tazuna |
| Update Description | Diff + template → LLM generation → GitHub API | tazuna (UI button) |
| Discard | Delete branch | tazuna (on user request) |

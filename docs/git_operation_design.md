...
This document defines the design policy for placing git operations (worktree, add, commit, push, etc.) in zloth.
...
A method where zloth centrally manages git operations, allowing AI Agents to **only edit files**.
...
flowchart TD
    ...
    subgraph "zloth Managed (Automatic)"
        B
        E
        G
        H
...
```python
# apps/api/src/zloth_api/services/git_service.py
...
```python
# apps/api/src/zloth_api/executors/base_executor.py

from abc import ABC, abstractmethod
from pathlib import Path
from collections.abc import Awaitable, Callable

from zloth_api.domain.models import AgentConstraints
...
class PRService:
    ...
    async def create_from_run(
        ...
        pr_response = await self.github_service.create_pull_request(
            ...
            body=pr_data.body or f"Generated by zloth\n\n{run.summary}",
        )
...
1. **Separation of Responsibilities**: AI Agent only edits files, zloth centrally manages git operations
...
|-------|-----------|----------|
| Preparation | git worktree add | zloth |
| Execution | File editing | AI Agent |
| Save | git add, commit, push | zloth (automatic) |
| Create PR | GitHub API: Create PR | zloth (on user request) |
| Update PR | Additional instruction → commit/push (auto-reflected) | zloth |
| Update Description | Diff + template → LLM generation → GitHub API | zloth (UI button) |
| Discard | Delete branch | zloth (on user request) |

...
zlothにおけるAIエージェント活用の効果測定と開発プロセスの可視化を目的としたメトリクス機能を提供する。
...
> **選定理由**: zlothの価値は「AIエージェントによる開発生産性向上」であり、実際にMergeされたPR数がその成果を最も直接的に表す。品質を伴わないスループットは意味がないため、Merge Rate や Review Score と併せて監視する。

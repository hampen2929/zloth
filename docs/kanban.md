...
zloth に看板ページを追加し、タスクの進捗を視覚的に管理できるようにする。
...
#### Task（`apps/api/src/zloth_api/domain/models.py`）
```python
...
```python
# apps/api/src/zloth_api/services/kanban_service.py
...
```python
# apps/api/src/zloth_api/routes/kanban.py
...
```python
# apps/api/src/zloth_api/services/github_service.py

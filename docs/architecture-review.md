...
#### 2. インメモリキューによるデータロスリスク（解消）

現状/課題: 一部の非同期処理（Breakdown、PRリンク生成）がインメモリ実行であり、プロセス再起動時にジョブや結果が失われるリスクがあった。

対応: 耐久キュー（SQLite バックド）へ完全移行。
- 追加: `JobKind.BREAKDOWN_EXECUTE`, `JobKind.PR_LINK_AUTO`
- ハンドラ: `get_job_worker()` に `breakdown.execute` と `pr.link.auto` を登録
- サービス: Breakdown/PR サービスは `JobDAO.create()` でキュー投入し、結果は `JobDAO.update_payload()` で保存
- ルート: `/prs/jobs/{job_id}` は DAO 経由でステータス/結果を返却

効果: 再起動・クラッシュ時もジョブ/結果が保持され、データロスが解消。起動時リカバリ（`recover_startup()`）と合わせて一貫性を確保。
...
| 品質属性 | 現状スコア | 目標スコア | 備考 |
|----------|------------|------------|------|
| **可用性** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | SQLiteバックドジョブキューで改善済み（Breakdown/PR Linkも移行） |
| **保守性** | ⭐⭐ | ⭐⭐⭐⭐ | リファクタリングで改善 |
| **拡張性** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | プラグイン構造は良好 |
| **テスト容易性** | ⭐⭐ | ⭐⭐⭐⭐ | DI改善・責務分離で対応 |

...
4. **AI に「コンフリクト解決」単体のタスクを委譲できる入口を用意する**
   - PR のマージベースでコンフリクト検知時、CLI/サーバーは以下を自動実行する
     1. 対象ファイルの三者比較（base / main / work）を収集
     2. 既存の「両方を残す」ポリシーや差分の意図をコンテキストに付与
     3. AI へ「統合後の最終ファイル」を生成させ、作業ブランチに反映
   - GitHub UI 依存を排除し、「依頼すれば解決まで完了する」体験に寄せる

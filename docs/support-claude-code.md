...
This document outlines the design and implementation plan for integrating Claude Code as an agent executor in zloth. Unlike the existing `PatchAgent` which generates patches via LLM API calls, the Claude Code workflow executes the Claude Code CLI directly within isolated git worktrees.
...
**File**: `apps/api/src/zloth_api/services/worktree_service.py`
...
**File**: `apps/api/src/zloth_api/executors/claude_code_executor.py`
...
**File**: `apps/api/src/zloth_api/services/run_service.py`
...
**File**: `apps/api/src/zloth_api/routes/runs.py`
...
3. **Feature flag**: Enable Claude Code executor via `ZLOTH_ENABLE_CLAUDE_CODE=true`
...
ZLOTH_ENABLE_CLAUDE_CODE=true

# Claude Code CLI path (optional, defaults to 'claude')
ZLOTH_CLAUDE_CLI_PATH=/usr/local/bin/claude

# Default timeout for Claude Code execution (seconds)
ZLOTH_CLAUDE_CODE_TIMEOUT=3600

# Maximum concurrent Claude Code runs
ZLOTH_CLAUDE_CODE_MAX_CONCURRENT=5
```
...
- [zloth Architecture](./architecture.md)
- [zloth Agents](./agents.md)

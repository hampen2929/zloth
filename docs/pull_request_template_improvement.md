...
対象リポジトリに `pull_request_template` が存在するにもかかわらず、tazuna で作成される PR 本文（PR Desc）がうまく生成・反映されないことがある。
...
バックエンド `apps/api/src/tazuna_api/services/pr_service.py` では、主に以下の3パスで PR 本文を作る。
...
- **再生成**: PR本文の「tazuna生成ブロック」だけを更新し、ユーザの手修正やテンプレ固定部は保持する
...
#### 方針C-1: tazuna管理ブロック（マーカー）方式（推奨）

テンプレ本文は一切削らず、本文の先頭（または適切な位置）に tazuna 管理ブロックを挿入する。
...
- **tazuna管理ブロックの中身のみ**（Summary/Changes/Test Plan など）
...
- **再生成**
  - tazunaブロックだけが更新され、ブロック外の手修正が残る
...
### Step 3: 非破壊合成（tazunaブロック方式）へ切り替え

- `_render_pr_body_from_template()` と `_fill_template_sections()` を「テンプレ破壊しない」方式に置換
- 生成文は `<!-- tazuna:begin --> ... <!-- tazuna:end -->` に閉じ込め、テンプレ本文は保持
- `regenerate_description` も同様に、tazunaブロックのみ更新する

### Step 4: LLMプロンプトの変更（ブロック生成に寄せる）

- LLMにはテンプレ全文の再生成をさせず、tazunaブロックに入れる内容だけを生成させる
- 可能なら「出力はMarkdownで、以下の見出しのみ」と制約を強める（例: Summary / Changes / Test Plan）

### Step 5: 既存動作の互換・段階導入

- テンプレ未指定でも今まで通り PR が作れる（デフォルト選択）
- 既存の PR 本文に tazunaブロックが無い場合は、初回再生成時に挿入して以後は更新する
...
対策として、テンプレを **列挙して選択可能**にし、本文は **非破壊な tazunaブロック方式**で合成・再生成する方針が、最も「どのような pull_request_template にも対応」しやすい。

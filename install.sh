...
# Generate docker-compose.local.yml for pre-built images (local development)
generate_compose_local() {
    print_step "Configuring pre-built images from registry..."

    cat > docker-compose.local.yml << EOF
# Auto-generated by install.sh for local development
# This file configures zloth to use pre-built images from GitHub Container Registry
# For production deployment with HTTPS, use docker-compose.prod.yml instead
version: '3.8'
...
start_services() {
    ...
    if [ "$BUILD_LOCAL" = true ]; then
        ...
        ZLOTH_API_PORT="$API_PORT" ZLOTH_WEB_PORT="$WEB_PORT" $compose_cmd -f docker-compose.local.yml pull
        ZLOTH_API_PORT="$API_PORT" ZLOTH_WEB_PORT="$WEB_PORT" $compose_cmd -f docker-compose.local.yml up -d
    fi
...
print_completion() {
    ...
    if [ "$BUILD_LOCAL" = true ]; then
        ...
        echo "    docker compose -f docker-compose.local.yml up -d     # Start services"
    fi
    echo ""
    echo "  For production deployment with HTTPS:"
    echo "    See docker-compose.prod.yml (requires Caddy setup)"
    echo ""
